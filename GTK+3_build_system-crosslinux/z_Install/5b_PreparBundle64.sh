...
rm -f "$PREFIX/bin/x86_64-w64-mingw32-pkg-config.exe"
...
mv -f "$PREFIX/lib64/libffi.dll.a" "$PREFIX/lib"
rm -rf "$PREFIX/lib64"
...
cp -f /usr/x86_64-w64-mingw32/sys-root/mingw/bin/pthreadGC2.dll "$PREFIX/bin"
cp -f "$WIN32PREFIX/lib/gtk-3.0/3.0.0/immodules.cache" "$PREFIX/lib/gtk-3.0/3.0.0/immodules.cache"
cp -f "$WIN32PREFIX/lib/gdk-pixbuf-2.0/2.10.0/loaders.cache" "$PREFIX/lib/gdk-pixbuf-2.0/2.10.0/loaders.cache"
mkdir "$PREFIX/etc/pango"
cp -f "$WIN32PREFIX/etc/pango/pango.modules" "$PREFIX/etc/pango/pango.modules"
cp -f "$BUILD"_bundle/settings.ini "$PREFIX/etc/gtk-3.0"

# add licensing information
cp -f "$BUILD"_bundle/doc/* "$PREFIX/share/doc"

# add "gnome-light" theme used by "gtk3-demo" and "gtk3-widget-factory"
#cp -f "$BUILD"_bundle/icons/* "$PREFIX/share/icons"

# generate MSVC import libraries
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/atk-1.0.def" /name:"$PREFIX/bin/libatk-1.0-0.dll" /out:"$PREFIX/lib/atk-1.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/cairo.def" /name:"$PREFIX/bin/libcairo-2.dll" /out:"$PREFIX/lib/cairo.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/fontconfig.def" /name:"$PREFIX/bin/libfontconfig-1.dll" /out:"$PREFIX/lib/fontconfig.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/gailutil.def" /name:"$PREFIX/bin/libgailutil-3-0.dll" /out:"$PREFIX/lib/gailutil.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/gdk-win32-3.0.def" /name:"$PREFIX/bin/libgdk-3-0.dll" /out:"$PREFIX/lib/gdk-win32-3.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/gdk_pixbuf-2.0.def" /name:"$PREFIX/bin/libgdk_pixbuf-2.0-0.dll" /out:"$PREFIX/lib/gdk_pixbuf-2.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/gio-2.0.def" /name:"$PREFIX/bin/libgio-2.0-0.dll" /out:"$PREFIX/lib/gio-2.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/glib-2.0.def" /name:"$PREFIX/bin/libglib-2.0-0.dll" /out:"$PREFIX/lib/glib-2.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/gmodule-2.0.def" /name:"$PREFIX/bin/libgmodule-2.0-0.dll" /out:"$PREFIX/lib/gmodule-2.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/gobject-2.0.def" /name:"$PREFIX/bin/libgobject-2.0-0.dll" /out:"$PREFIX/lib/gobject-2.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/gthread-2.0.def" /name:"$PREFIX/bin/libgthread-2.0-0.dll" /out:"$PREFIX/lib/gthread-2.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/gtk-win32-3.0.def" /name:"$PREFIX/bin/libgtk-3-0.dll" /out:"$PREFIX/lib/gtk-win32-3.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/pango-1.0.def" /name:"$PREFIX/bin/libpango-1.0-0.dll" /out:"$PREFIX/lib/pango-1.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/pangocairo-1.0.def" /name:"$PREFIX/bin/libpangocairo-1.0-0.dll" /out:"$PREFIX/lib/pangocairo-1.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/pangoft2-1.0.def" /name:"$PREFIX/bin/libpangoft2-1.0-0.dll" /out:"$PREFIX/lib/pangoft2-1.0.lib"
wine "$BUILD"_bundle/lib.exe /machine:X64 /def:"$PREFIX/lib/pangowin32-1.0.def" /name:"$PREFIX/bin/libpangowin32-1.0-0.dll" /out:"$PREFIX/lib/pangowin32-1.0.lib"
rm -f "$PREFIX/lib/"*.exp
...
# copy the README
cp -f "$BUILD"_bundle/gtk+-bundle_CONTINUOUS_DEBUG-XXXXXXXX_"$BUILD".README.txt "$PREFIX/gtk+-bundle_CONTINUOUS_DEBUG-`date +%Y%m%d`_"$BUILD".README.txt"
...
# zip the whole thing
cd "$PREFIX"
zip -r gtk+-bundle_CONTINUOUS_DEBUG-`date +%Y%m%d`_"$BUILD".zip bin/ etc/ include/ lib/ manifest/ share/ src/ gtk+-bundle_CONTINUOUS_DEBUG-`date +%Y%m%d`_"$BUILD".README.txt
mv -f gtk+-bundle_CONTINUOUS_DEBUG-`date +%Y%m%d`_"$BUILD".zip /srv/win32builder/releases
